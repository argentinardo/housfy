<script lang="ts">
  ...
  import { AlertType, type ShowAlertEventDetail, SHOW_ALERT_EVENT } from '../utils/simpleAlertUtils';
  ...
  onMount(() => {
    document.addEventListener(SHOW_ALERT_EVENT, handleShowAlert as EventListener);
  });
  
  onDestroy(() => {
    document.removeEventListener(SHOW_ALERT_EVENT, handleShowAlert as EventListener);
  });
  ...
  function getAlertClass(type: AlertType): string {
    switch(type) {
      ...
      case AlertType.ADVICE: return 'alert-advice';
  ...
  {#each activeAlerts as alert (alert.id)}
    ...
    >
      <div class="alert-content">
        ...
        <button class="close-button" on:click={() => removeAlert(alert.id)}>×</button>
      </div>
    </div>
  {/each}
</div>
